...
v1 = pd.read_csv("C:\\Users\\WIN10\\Desktop\\LEARNING\\DS\\LassoRidge\\Computer_Data.csv")
...
model_train = smf.ols("price ~ speed + hd + ram", data = v1).fit()
model_train.summary()
# Prediction
...
# Adjusted r-square
lasso.score(v1.iloc[:, 1:], v1.price)
# RMSE
np.sqrt(np.mean((pred_lasso - v1.price)**2))
# RIDGE REGRESSION 
from sklearn.linear_model import Ridge
rm = Ridge(alpha = 0.4, normalize = True)
rm.fit(v1.iloc[:, 1:], v1.price)
...
# Adjusted r-square
rm.score(v1.iloc[:, 1:], v1.price)
# RMSE
np.sqrt(np.mean((pred_rm - v1.price)**2))
# ELASTIC NET REGRESSION 
from sklearn.linear_model import ElasticNet 

enet = ElasticNet(alpha = 0.4)
enet.fit(v1.iloc[:, 1:], v1.price) 
# Coefficients values for all the independent vairbales
enet.coef_
enet.intercept_
...
# Adjusted r-square
enet.score(v1.iloc[:, 1:], v1.price)
# RMSE
np.sqrt(np.mean((pred_enet - v1.price)**2))
...
lasso_reg.fit(v1.iloc[:, 1:], v1.price)
lasso_reg.best_params_
lasso_reg.best_score_
lasso_pred = lasso_reg.predict(v1.iloc[:, 1:])
# Adjusted r-square#
lasso_reg.score(v1.iloc[:, 1:], v1.price)
# RMSE
np.sqrt(np.mean((lasso_pred - v1.price)**2))
...
ridge_reg.fit(v1.iloc[:, 1:], v1.price)
ridge_reg.best_params_
ridge_reg.best_score_
ridge_pred = ridge_reg.predict(v1.iloc[:, 1:])
# Adjusted r-square#
ridge_reg.score(v1.iloc[:, 1:], v1.price)
# RMSE
np.sqrt(np.mean((ridge_pred - v1.price)**2))
...
enet_reg.fit(v1.iloc[:, 1:], v1.price)

enet_reg.best_params_
enet_reg.best_score_
enet_pred = enet_reg.predict(v1.iloc[:, 1:])
# Adjusted r-square
enet_reg.score(v1.iloc[:, 1:], v1.price)
# RMSE
np.sqrt(np.mean((enet_pred - v1.price)**2))
